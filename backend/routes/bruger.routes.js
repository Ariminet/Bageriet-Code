  ...
  try {
    ...
    if (bruger) {
      ...
      return res.status(401).json({
        message:
          'Bruger findes allerede (OBS - denne besked skal laves om - GDPR!',
      });
    } else {
...
router.delete('/admin/:id', findBruger, async (req, res) => {
  ...
  if (res.bruger.rolle != 'ADMIN') {
    try {
      ...
      res.status(500).json({
        message:
          'Der kan ikke slettes - der er opstået en fejl: ' + error.message,
      });
    }
  } else {
...
router.put('/admin/:id', findBruger, async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({
      message: 'Der kan ikke rettes - der er opstået en fejl: ' + error.message,
    });
  }
});
...
async function findBruger(req, res, next) {
  ...
  try {
    ...
    if (bruger == null) {
      return res
        .status(404)
        .json({ message: 'Der findes ikke en  med den ID' });
    }
  } catch (error) {
    console.log(error);
