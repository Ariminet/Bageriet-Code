...
router.get('/antal/:antal', async (req, res) => {
  ...
  try {
    ...
    const produkter = await Produkt.find(
      {},
      'titel teaser image pris oprettet kommentar'
    )
      .sort([['oprettet', -1]])
      .limit(antal);
    res.json(produkter);
    ...
    const produkter = await Produkt.find(
      {
        $or: [
          ...
          {
            'ingredienser.ingrediens_titel': {
              $regex: req.params.soegeord,
              $options: 'i',
            },
          },
        ],
...
router.get('/:id', async (req, res) => {
  ...
  produkt = await Produkt.findById(req.params.id).populate([
    { path: 'kategori' },
    {
      path: 'kommentar',
      model: Kommentar,
      populate: { path: 'bruger', model: Bruger },
    },
  ]); // {path: 'kommentar'},
  res.json(produkt);
});
...
router.patch('/likes/:id', async (req, res) => {
  ...
  try {
    await Produkt.findById(req.params.id, async function (err, p) {
      ...
      res
        .status(201)
        .json({ message: 'Der er tilføjet 1 like', antal_likes: p.likes });
    });
  } catch (error) {
...
router.delete('/admin/:id', findProdukt, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: 'Kan ikke slettes - der er opstået en fejl: ' + error.message,
    });
    //res.status(500).end();
  }
});
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(400).json({
        message: 'Kan ikke rettes - der er opstået en fejl: ' + error.message,
      });
      //res.status(400).end();
    }
  }
